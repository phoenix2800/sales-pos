...
router.post('/income', authMiddleware, (req, res) => {
  try {
    ...
    const result = db.prepare(
      'INSERT INTO treasury (date, type, amount, description, category, user_id) VALUES (?, ?, ?, ?, ?, ?)'
    ).run(date, 'income', parseFloat(amount), description || null, category || 'other', req.user.id);

    const entry = db.prepare('SELECT * FROM treasury WHERE id = ?').get(result.lastInsertRowid);
...
router.post('/expense', authMiddleware, (req, res) => {
  try {
    ...
    const result = db.prepare(
      'INSERT INTO treasury (date, type, amount, description, category, user_id) VALUES (?, ?, ?, ?, ?, ?)'
    ).run(date, 'expense', parseFloat(amount), description || null, category || 'other', req.user.id);
...
router.get('/monthly', authMiddleware, (req, res) => {
  try {
    ...
    const totalIncome = db.prepare("SELECT COALESCE(SUM(amount), 0) as total FROM treasury WHERE type = 'income' AND date BETWEEN ? AND ?").get(start, end).total;
    const totalExpense = db.prepare("SELECT COALESCE(SUM(amount), 0) as total FROM treasury WHERE type = 'expense' AND date BETWEEN ? AND ?").get(start, end).total;
...
router.get('/entries/list', authMiddleware, (req, res) => {
  try {
    ...
    const total = db.prepare(query.replace(/SELECT[\s\S]*?FROM/, 'SELECT COUNT(*) as total FROM')).get(...params).total;
...
router.get('/export/daily', authMiddleware, (req, res) => {
  try {
    const { date } = req.query;
    const targetDate = date || format(new Date(), 'yyyy-MM-dd');
    const entries = db.prepare('SELECT * FROM treasury WHERE date = ? ORDER BY created_at').all(targetDate);
