...
router.post('/signup', authMiddleware, adminOnly, async (req, res) => {
  try {
    ...
    const cookieOpts = { httpOnly: false, sameSite: 'Lax', maxAge: 7*24*60*60*1000, path: '/' };
...
router.post('/login', async (req, res) => {
  try {
    ...
    // Set cookie (works for popup windows on mobile where Authorization header can't be sent)
    const cookieOpts = {
      httpOnly: false,
      sameSite: 'Lax',
    ...
    // Only use Secure flag on actual HTTPS (Render uses x-forwarded-proto)
