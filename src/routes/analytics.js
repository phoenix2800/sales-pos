...
router.get('/sales/monthly', authMiddleware, (req, res) => {
  try {
...
router.get('/profit-loss', authMiddleware, adminOnly, (req, res) => {
  try {
    const { month, year } = req.query;
    const now = new Date();
    const m = month ? parseInt(month) - 1 : now.getMonth();
...
router.get('/inventory', authMiddleware, adminOnly, (req, res) => {
  try {
    const products = db.prepare('SELECT * FROM products WHERE is_active = 1 ORDER BY quantity ASC').all();
