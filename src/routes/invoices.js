...
router.get('/', authMiddleware, async (req, res) => {
  try {
    ...
    query += ' GROUP BY i.id, c.id, u.id ORDER BY i.date DESC, i.time DESC';
...
router.get('/:id/download-pdf', authMiddleware, async (req, res) => {
  try {
    ...
    let filePath = invoice.pdf_path ? path.join(__dirname, '../..', invoice.pdf_path) : null;
    // On ephemeral hosting (Render free) the cached PDF may be gone → regenerate from DB data
    if (!filePath || !fs.existsSync(filePath)) {
      const client = await getDb().get('SELECT * FROM clients WHERE id = ?', [invoice.client_id]);
      const items = await getDb().all(`
        SELECT id.*, p.name as product_name FROM invoice_details id
        LEFT JOIN products p ON id.product_id = p.id WHERE id.invoice_id = ?
      `, [id]);
      const pdfPath = await generateInvoicePDF(invoice, client || { name: '' }, items);
      await getDb().run('UPDATE invoices SET pdf_path = ? WHERE id = ?', [pdfPath, id]).catch(() => {});
...
// Restores previous stock, validates new items, adjusts stock + treasury + debts
router.put('/:id', authMiddleware, async (req, res) => {
  const { id } = req.params;
  const db = getDb();
  try {
    const existing = await db.get('SELECT * FROM invoices WHERE id = ? AND is_archived = 0', [id]);
    if (!existing) return res.status(404).json({ error: 'Invoice not found' });

    // Non-admin can only edit very recent invoices (same day, their own)
    if (req.user.role !== 'admin' && (existing.user_id !== req.user.id || existing.date !== format(new Date(), 'yyyy-MM-dd'))) {
      return res.status(403).json({ error: 'You can only edit your own invoices from today' });
    }

    const { client_id, items = [], amount_paid, notes, payment_type } = req.body;
    if (!client_id) return res.status(400).json({ error: 'Client is required' });
    ...
    // ======= 1. Revert old invoice's effects =======
    // Restore stock from old items
    const oldItems = await db.all('SELECT * FROM invoice_details WHERE invoice_id = ?', [id]);
    for (const oi of oldItems) {
      await db.run('UPDATE products SET quantity = quantity + ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [oi.quantity, oi.product_id]);
    }
    // Remove old treasury entries tied to this invoice
    await db.run("DELETE FROM treasury WHERE reference = ? AND category = 'sales'", [existing.invoice_number]);
    // Remove old debt if any
    const oldDebt = await db.get('SELECT * FROM debts WHERE invoice_id = ?', [id]);
    if (oldDebt) {
      await db.run('UPDATE clients SET total_debt = total_debt - ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [oldDebt.amount || 0, existing.client_id]);
      await db.run('DELETE FROM debts WHERE invoice_id = ?', [id]);
    }
    ...
    for (const item of items) {
      ...
      const newQty = product.quantity - quantity;
      await db.run('UPDATE products SET quantity = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [newQty, product.id]);
      await db.run('INSERT INTO stock_history (product_id, quantity_before, quantity_after, change_type, quantity_change, reference, user_id) VALUES (?, ?, ?, ?, ?, ?, ?)',
        [product.id, product.quantity, newQty, 'sale-edit', -quantity, existing.invoice_number + ' (edited)', req.user.id]);
    }

    const paid = Math.min(parseFloat(amount_paid !== undefined ? amount_paid : existing.amount_paid) || 0, total_amount);
    const amount_debt = total_amount - paid;
    let status = 'paid';
    if (paid === 0) status = 'unpaid';
    else if (paid < total_amount) status = 'partial';
    ...
    // ======= 3. Update invoice =======
    await db.run(
      `UPDATE invoices SET client_id = ?, total_amount = ?, amount_paid = ?, amount_debt = ?, status = ?, notes = ? WHERE id = ?`,
      [client_id, total_amount, paid, amount_debt, status, notes || null, id]
    );

    // Replace items
    await db.run('DELETE FROM invoice_details WHERE invoice_id = ?', [id]);
    for (const it of validatedItems) {
      await db.run('INSERT INTO invoice_details (invoice_id, product_id, quantity, unit_price, purchase_price, total_price, profit_margin) VALUES (?, ?, ?, ?, ?, ?, ?)',
        [id, it.product_id, it.quantity, it.unit_price, it.purchase_price, it.total_price, it.profit_margin]);
    }

    // ======= 4. Re-record treasury + debt =======
    const date = existing.date;
    if (paid > 0) {
      await db.run('INSERT INTO treasury (date, type, amount, description, category, reference, user_id) VALUES (?, ?, ?, ?, ?, ?, ?)',
        [date, 'income', paid, `Sales invoice ${existing.invoice_number} (edited)`, 'sales', existing.invoice_number, req.user.id]);
    }
    if (amount_debt > 0) {
      const dueDateStr = format(new Date().getTime() + 30*86400000, 'yyyy-MM-dd');
      await db.run('INSERT INTO debts (client_id, invoice_id, amount, original_amount, date, due_date, status, paid_amount) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        [client_id, id, amount_debt, amount_debt, date, dueDateStr, 'unpaid', 0]);
      await db.run('UPDATE clients SET total_debt = total_debt + ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [amount_debt, client_id]);
    }

    // Regenerate PDF
    try {
      const fullInv = await db.get('SELECT * FROM invoices WHERE id = ?', [id]);
      const pdfPath = await generateInvoicePDF(fullInv, client, validatedItems);
      await db.run('UPDATE invoices SET pdf_path = ? WHERE id = ?', [pdfPath, id]);
    } catch(e) { console.error('PDF regenerate:', e.message); }

    // Emit sockets
    const io = req.app.get('io');
    if (io) {
      io.emit('invoice:updated', { id });
      io.emit('stock:updated');
    }

    res.json({ message: 'Invoice updated', invoice: { id, total_amount, amount_paid: paid, amount_debt, status, items: validatedItems } });
  } catch (error) {
