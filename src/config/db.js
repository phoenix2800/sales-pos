...
const DB_TYPE = (() => {
  const t = (process.env.DATABASE_TYPE || '').toLowerCase();
  if (t === 'postgres' || t === 'postgresql') return 'postgres';
  if (process.env.DATABASE_URL && /^postgres(ql)?:\/\//i.test(process.env.DATABASE_URL)) return 'postgres';
  return 'sqlite';
})();
...
function convertParams(sql, params) {
  ...
  // SQLite LIKE is case-insensitive, PostgreSQL LIKE is not → use ILIKE for identical behavior.
  let idx = 0;
  const pgSql = sql
    .replace(/\?/g, () => `$${++idx}`)
...
async function initPostgres() {
  ...
  // Return the same JS types as the SQLite layer, so route code and the
  // prebuilt frontend behave identically in both modes.
  types.setTypeParser(20, (v) => (v === null ? null : parseInt(v, 10))); // int8 (COUNT/SUM of ints) → number
  types.setTypeParser(1700, (v) => (v === null ? null : parseFloat(v))); // numeric → number
  types.setTypeParser(1082, (v) => v); // DATE → 'YYYY-MM-DD' string (not a Date object)
  const tsParser = (v) => (v === null ? null : String(v).replace('T', ' ').slice(0, 19)); // → 'YYYY-MM-DD HH:MM:SS'
  types.setTypeParser(1114, tsParser); // TIMESTAMP
  types.setTypeParser(1184, tsParser); // TIMESTAMPTZ
  const pool = new Pool({
